/* 
 * This script sets up ALOS/PALSAR-1 and ALOS/PALSAR-2 images over Southern Negros Island
 * Philippines at three time points for the years 2007, 2010, and 2015. The SAR images were
 * classified at Level 1 of a multi-level, hierarchical classification system with 5 classes
 * namely: Forestland, Settlements, Cropland, Wetland, Grassland, Other land.
 */
...
Map.setCenter(123.1, 9.25, 10); // Zoom in and center display to study area

// Define and display box covering extent of PALSAR tile
var box = ee.Geometry.Rectangle(122.85,9.51, 123.335,9.02);
...
var hh2007 = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_2007_PALSAR_HH_Sigma0').clip(box); // 2007 HH polarisation
var hv2007 = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_2007_PALSAR_HV_Sigma0').clip(box); // 2007 HV polarisation
var hh2010 = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_2010_PALSAR_HH_Sigma0').clip(box); // 2010 HH polarisation
var hv2010 = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_2010_PALSAR_HV_Sigma0').clip(box); // 2010 HV polarisation
var hh2015 = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_2015_PALSAR_HH_Sigma0').clip(box); // 2015 HH polarisation
var hv2015 = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_2015_PALSAR_HV_Sigma0').clip(box); // 2015 HV polarisation

// Load combined mask image
var coMask = ee.Image('users/dondealban/Philippines/ALOSKC4/SNG/SNegros_PALSAR_Mask_Combined').clip(box); // Combined Mask
...
// Load land cover points
var points = ee.FeatureCollection('users/dondealban/Philippines/ALOSKC4/SNG/snegros-landcover-roi-final');
Map.addLayer(points, {'color': '1E90FF'}, 'Southern Negros ROI');

// Create 25m-radius buffer around points
var buff25 = function(feature) {
    return feature.buffer(25);  
};
var snegrosROI = points.map(buff25);

// Initialise random column and values for ROI feature collection 
snegrosROI = snegrosROI.randomColumn('random1', seed);

var train = snegrosROI.filter(ee.Filter.lte('random1', 0.7));
var test  = snegrosROI.filter(ee.Filter.gt('random1', 0.7));
...
var em2007 = validation2007.errorMatrix('ClassID2', 'classification'); // Error matrix
...
var classPalette = ['ffffff',  // No data
                    ...
                    'ffff66',  // Grassland
                    'b3b3b3']; // Other land
Map.addLayer(masked2007, {min: 0, max: 6, palette: classPalette}, '2007 Classification',  true);
Map.addLayer(masked2010, {min: 0, max: 6, palette: classPalette}, '2010 Classification',  true);
Map.addLayer(masked2015, {min: 0, max: 6, palette: classPalette}, '2015 Classification',  true);
...
Export.image.toDrive({
  ...
  description: 'Classification_SNegros_2007_L2',
...
Export.image.toDrive({
  ...
  description: 'Classification_SNegros_2010_L2',
  folder: 'Google Earth Engine',
  region: box,
  scale: 25,
  maxPixels: 300000000,
...
Export.image.toDrive({
  ...
  description: 'Classification_SNegros_2015_L2',
  folder: 'Google Earth Engine',
  region: box,
  scale: 25,
  maxPixels: 300000000,
...
Export.table.toDrive(roi2007train, 'ROI_Train_SNegros_2007_L2', 'Google Earth Engine');
Export.table.toDrive(roi2010train, 'ROI_Train_SNegros_2010_L2', 'Google Earth Engine');
Export.table.toDrive(roi2015train, 'ROI_Train_SNegros_2015_L2', 'Google Earth Engine');
Export.table.toDrive(roi2007test, 'ROI_Test_SNegros_2007_L2', 'Google Earth Engine');
Export.table.toDrive(roi2010test, 'ROI_Test_SNegros_2010_L2', 'Google Earth Engine');
Export.table.toDrive(roi2015test, 'ROI_Test_SNegros_2015_L2', 'Google Earth Engine');
Export.table.toDrive(validation2007, 'Validation_SNegros_2007_L2', 'Google Earth Engine');
Export.table.toDrive(validation2010, 'Validation_SNegros_2010_L2', 'Google Earth Engine');
Export.table.toDrive(validation2015, 'Validation_SNegros_2015_L2', 'Google Earth Engine');
...
Export.table.toDrive({
  ...
  description: 'AA_SNegros_2007_L2',
  fileFormat: 'CSV',
  folder: 'Google Earth Engine',
});
...
Export.table.toDrive({
  ...
  description: 'AA_SNegros_2010_L2',
  fileFormat: 'CSV',
  folder: 'Google Earth Engine',
});
...
Export.table.toDrive({
  ...
  description: 'AA_SNegros_2015_L2',
  fileFormat: 'CSV',
  folder: 'Google Earth Engine',
});
