/*
 * This script sets up ALOS/PALSAR-1 and ALOS/PALSAR-2 images over Northern Negros Island 
 * Province, Philippines at three time points for the years 2007, 2010, and 2015. The SAR 
 * images were classified at Level 4 of a multi-level, hierarchical classification system
 * with 10 classes namely: Closed forest, broadleaved; Open forest, broadleaved; Mangrove
 * forest; Other land, built-up area; Other land, cultivated, annual crop; Other land, 
 * cultivated, perennial crop; Inland water; Other land, natural, grassland; Other wooded
 * land, shrubs; Other wooded land, wooded grassland.
 */
...
Map.setCenter(123.20, 10.62, 10); // Zoom in and center display to study area
...
var hh2007 = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_2007_PALSAR_HH_Sigma0').clip(box); // 2007 HH polarisation
var hv2007 = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_2007_PALSAR_HV_Sigma0').clip(box); // 2007 HV polarisation
var hh2010 = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_2010_PALSAR_HH_Sigma0').clip(box); // 2010 HH polarisation
var hv2010 = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_2010_PALSAR_HV_Sigma0').clip(box); // 2010 HV polarisation
var hh2015 = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_2015_PALSAR_HH_Sigma0').clip(box); // 2015 HH polarisation
var hv2015 = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_2015_PALSAR_HV_Sigma0').clip(box); // 2015 HV polarisation

// Load combined mask image
var coMask = ee.Image('users/dondealban/Philippines/ALOSKC4/NNG/NNegros_PALSAR_Mask_Combined').clip(box); // Combined Mask
...
Map.addLayer(points, {'color': '1E90FF'}, 'Northern Negros ROI');

// Create 25m-radius buffer around points
var buff25 = function(feature) {
    return feature.buffer(25);  
};
var nnegrosROI = points.map(buff25);

// Initialise random column and values for ROI feature collection 
nnegrosROI = nnegrosROI.randomColumn('random1', seed);

var train = nnegrosROI.filter(ee.Filter.lte('random1', 0.7));
var test  = nnegrosROI.filter(ee.Filter.gt('random1', 0.7));
...
var em2007 = validation2007.errorMatrix('ClassID4', 'classification'); // Error matrix
...
// Calculate accuracy metrics
var em2010 = validation2010.errorMatrix('ClassID4', 'classification'); // Error matrix
...
// Calculate accuracy metrics
var em2015 = validation2015.errorMatrix('ClassID4', 'classification'); // Error matrix
...
var classPalette = ['ffffff',  // No data
                    '246a24',  // Closed forest, broadleaved
                    'ccff66',  // Open forest, broadleaved
                    '84d39e',  // Forest plantation, broadleaved
                    '6666ff',  // Mangrove forest
                    'ff0000',  // Other land, built-up area
                    '966400',  // Other land, cultivated, annual crop
                    '784b00',  // Other land, cultivated, perennial crop
                    'fb9a99',  // Other land, fishpond
                    '66ccff',  // Inland water
                    'ffff66',  // Other land, natural, grassland
                    'cfcfcf',  // Other wooded land, fallow
                    'ffd278',  // Other wooded land, shrubs
                    'ffb432']; // Other wooded land, wooded grassland
Map.addLayer(masked2007, {min: 0, max: 11, palette: classPalette}, '2007 Classification',  true);
Map.addLayer(masked2010, {min: 0, max: 11, palette: classPalette}, '2010 Classification',  true);
Map.addLayer(masked2015, {min: 0, max: 11, palette: classPalette}, '2015 Classification',  true);
...
Export.image.toDrive({
  ...
  description: 'Classification_NNegros_2007_L4',
...
Export.image.toDrive({
  ...
  description: 'Classification_NNegros_2010_L4',
  folder: 'Google Earth Engine',
  region: box,
  scale: 25,
  maxPixels: 300000000,
...
Export.image.toDrive({
  ...
  description: 'Classification_NNegros_2015_L4',
  folder: 'Google Earth Engine',
  region: box,
  scale: 25,
  maxPixels: 300000000,
...
Export.table.toDrive(roi2007train, 'ROI_Train_NNegros_2007_L4', 'Google Earth Engine');
Export.table.toDrive(roi2010train, 'ROI_Train_NNegros_2010_L4', 'Google Earth Engine');
Export.table.toDrive(roi2015train, 'ROI_Train_NNegros_2015_L4', 'Google Earth Engine');
Export.table.toDrive(roi2007test, 'ROI_Test_NNegros_2007_L4', 'Google Earth Engine');
Export.table.toDrive(roi2010test, 'ROI_Test_NNegros_2010_L4', 'Google Earth Engine');
Export.table.toDrive(roi2015test, 'ROI_Test_NNegros_2015_L4', 'Google Earth Engine');
Export.table.toDrive(validation2007, 'Validation_NNegros_2007_L4', 'Google Earth Engine');
Export.table.toDrive(validation2010, 'Validation_NNegros_2010_L4', 'Google Earth Engine');
Export.table.toDrive(validation2015, 'Validation_NNegros_2015_L4', 'Google Earth Engine');
...
Export.table.toDrive({
  ...
  description: 'AA_NNegros_2007_L4',
  fileFormat: 'CSV',
  folder: 'Google Earth Engine',
});
...
Export.table.toDrive({
  ...
  description: 'AA_NNegros_2010_L4',
  fileFormat: 'CSV',
  folder: 'Google Earth Engine',
});
...
Export.table.toDrive({
  ...
  description: 'AA_NNegros_2015_L4',
  fileFormat: 'CSV',
  folder: 'Google Earth Engine',
});
